 ...
 * @author Om
 */
public class LoginSellerCustomerController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try
        {
             boolean result=MembersDAO.findUser(member);
             if(result)
        {
                           response.setContentType("text/plain");
            response.getWriter().write("Show_Prod_Cust.jsp?name="+username.toUpperCase());
        }
             else
             {
                 response.setContentType("text/plain");
